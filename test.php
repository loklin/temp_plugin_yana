...
//Acces a la base du plugin pour recup des mesures.
	    $depart = date("Y-m-d");
            $startDate = strtotime ('-6 days', strtotime($depart));
	...
	$emetteurArray = array();
		while($row = $results->fetchArray()){
                ...
                if (!isset($emetteurArray[$row['Id']])) {
                    $emetteurArray[$row['Id']] = [$row['description'], $curTimestamp, $row['Mesure']];
                } 
...
//TODO ajouter la localisation à la base de donné plugin ou acceder a celle de yana des le debut et mettre dans l'array
...
// Les scripts TODO voir avec yana comment elle les preferes
        ?>
                ...
                $(".responsive-calendar").responsiveCalendar({
                    ...
                    events: {
                        ...
                        while ($row = $results->fetchArray()) {
                            ...
                            $listRecords .= '{"url":"javascript:window.location.href = \'test.php?date=';
                            $listRecords .= $row['recordDay'].'\'"},';
                        }
